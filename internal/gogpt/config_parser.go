...
func ParseConfig(src string) (Config, error) {
	...
	for {
		...
		case "template":
			...
			cfg.Templates = append(cfg.Templates, TemplateConfig{Name: item.name, Template: item.value})
		case "system":
			item, err := p.readNamedString()
			if err != nil {
				return Config{}, err
			}
			cfg.SystemMessages = append(cfg.SystemMessages, SystemMessageConfig{Name: item.name, Message: item.value})
		default:
...
func assignModelValue(model *ModelConfig, key string, value any) error {
	...
	case "classname":
		v, ok := value.(string)
		if !ok {
			return fmt.Errorf("classname must be a string")
		}
		model.ClassName = v
	default:
		if model.Params == nil {
